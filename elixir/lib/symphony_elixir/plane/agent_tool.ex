defmodule SymphonyElixir.Plane.AgentTool do
  @moduledoc """
  Provider-native Plane REST tool exposed to Codex app-server turns.
  """
  ...
  Execute a Plane REST API request using Symphony's configured auth.
  """
  @plane_api_input_schema %{
    ...
    "properties" => %{
      "method" => %{
        ...
        "description" => "Plane REST method."
      },
      "path" => %{
        "type" => "string",
        "description" => "Plane REST path such as /api/v1/workspaces/plane/projects/."
      },
      "params" => %{
        "type" => ["object", "null"],
  ...
  def execute(tool, arguments, opts) do
    case tool do
      @plane_api_tool -> execute_plane_api(arguments, opts)
  ...
  defp tool_error_payload(:invalid_arguments) do
    %{"error" => %{"message" => "`plane_api` expects an object with `method` and `path`."}}
  end

  defp tool_error_payload(:invalid_method) do
    %{"error" => %{"message" => "`plane_api.method` must be GET, POST, PATCH, PUT, or DELETE."}}
  end

  defp tool_error_payload(:invalid_path) do
    %{"error" => %{"message" => "`plane_api.path` must be a relative Plane REST path."}}
  end

  defp tool_error_payload(:invalid_params) do
    %{"error" => %{"message" => "`plane_api.params` must be a JSON object when provided."}}
  end

  defp tool_error_payload(:missing_plane_api_key) do
    %{
      "error" => %{
        "message" => "Symphony is missing Plane auth. Set `tracker.provider.api_key` in `WORKFLOW.md` or export `PLANE_API_KEY`."
      }
    }
  end
  ...
  defp tool_error_payload(reason) do
    %{"error" => %{"message" => "Plane API tool execution failed.", "reason" => inspect(reason)}}
  end
